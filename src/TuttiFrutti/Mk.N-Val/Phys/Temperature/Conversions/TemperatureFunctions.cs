...
{
    protected static readonly N _NoffsetK2C = N.CreateChecked(MathFixed.KminusC);
    ...
    private Dictionary<(Unit from, Unit to), Func<N, N>> _functions = new() {
        ...
        { (from: Unit.Kelvin, to: Unit.Fahrenheit), x =>  N.CreateChecked((double.CreateChecked(x) - MathFixed.KminusC)  * 9 / 5) + _N32  },
    };
