...
{
    protected static readonly N _NoffsetK2C = N.CreateChecked(Univ.KminusC);
    ...
    private Dictionary<(In from, In to), Func<N, N>> _functions = new() {
        ...
        { (from: In.Kelvin, to: In.Fahrenheit), x =>  N.CreateChecked((double.CreateChecked(x) - Univ.KminusC) / 5 * 9) + _N32  },
    };
