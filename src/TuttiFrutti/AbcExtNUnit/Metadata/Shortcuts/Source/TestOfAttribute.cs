...
{
    private static IEnumerable<Type> OfTypes => _ofTypes ?? _typeParameters
        .TakeWhile(x => x != _ofTypeStub).ToArray();

    private static readonly IEnumerable<Type>? _ofTypes = null;

    private static readonly Type[] _typeParameters = new Type[] { typeof(T1), typeof(T2), typeof(T3), typeof(T4),
